...
parVIEW::parVIEW(int argc, char** argv, QWidget *parent)
	: QMainWindow(parent)
...
// 	connect(pinfoAct, SIGNAL(triggered()), this, SLOT(openPinfoDialog()));
	...
	makePolyAct = new QAction(QIcon(":/image/icPolygon.png"), tr("&Create Polygon Object"), this);
	makePolyAct->setStatusTip(tr("Create Polygon Object"));
	connect(makePolyAct, SIGNAL(triggered()), this, SLOT(makePolygon()));

	makeCylinderAct = new QAction(QIcon(":/image/cylinder.png"), tr("&Create Cylinder Object"), this);
	...
	makeMassAct = new QAction(QIcon(":/image/mass.png"), tr("&Create mass"), this);
	...
	changeParticleAct = new QAction(QIcon(":/image/icChangeParticle.png"), tr("&Change particle from file"), this);
	...
	simPlayAct = new QAction(QIcon(":/image/ani_play.png"), tr("&simulation"), this);
...
void parVIEW::setAnimationAction(bool b)
{
...
void parVIEW::sim_play()
{
	disconnect(simPlayAct);
	simPlayAct->setIcon(QIcon(":/image/ani_pause.png"));
...
void parVIEW::sim_stop()
{
	disconnect(simPlayAct);
	simPlayAct->setIcon(QIcon(":/image/ani_play.png"));
	simPlayAct->setStatusTip(tr("Restart for simulation."));
	connect(simPlayAct, SIGNAL(triggered()), this, SLOT(sim_play()));
	//simPlayAct->setEnabled(true);
...
void parVIEW::ChangeParticleFromFile()
{
...
void parVIEW::DEMRESULTASCII_Export()
{
	ExportDialog edlg(md);
	edlg.callDialog();
}

void parVIEW::MBDRESULTASCII_Export()
{
 	QString dir = md->modelPath();
...
void parVIEW::OBJPROPERTY_Dialog()
{
	if (!ptObj)
...
void parVIEW::makePolygon()
{
//	polygonDialog pd;
//	gl->makePolygon(pd.callDialog(md));
}

void parVIEW::makeCylinder()
{
	cylinderDialog cyd;
...
void parVIEW::recieveProgress(unsigned int pt)
{
	pBar->setValue(pt);
	//pBar->update();
}

void parVIEW::deleteFileByEXT(QString ext)
{
