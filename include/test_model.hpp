...
public:
	...
	//	model::setGravity(9.80665, MINUS_Y);
		...
		VEC3D spi = ground->toLocal(VEC3D(0.0, 0.65, 1.0) - ground->Position());
		VEC3D spj = h_body->toLocal(VEC3D(0.0, 0.65, 1.0) - h_body->Position());
		...
		pointMass* wheel = mbd->PointMass("wheel1");
		...
		kinematicConstraint* rev = mbd->createKinematicConstraint(
  		...
  		drivingConstraint* h_dc = mbd->createDrivingConstraint(
  			"h_dc", h_trans, drivingConstraint::DRIVING_TRANSLATION, 0.0, 1.0);// drivingConstraint("h_dc");
