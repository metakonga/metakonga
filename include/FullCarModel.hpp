...
public:
	...
	bool setUp()
	{
		...
		VEC3D G_CarBody_Fix_location(0, 0, 0);
		...
		VEC3D LF_N_W_REV_location(-0.98833311, -0.67800231, -0.16953447);
		...
		VEC3D RR_SUS_UPPER_location(0.68165, 0.31627, 0.27691);
		...
		VEC3D LF_Nuckle_Steer_Bar_SPH_location(-1.08, -0.566562414, -0.14704955
			);
		...
		// CarBody
		rigidBody* CarBody = createRigidBody("CarBody", 2463.26396293451,
		...
		rigidBody* LF_wheel = createRigidBody(
			"LF_wheel", 28.05721,
		...
		springDamperModel* RF_SUS = createSpringDamperElement(
			...
			CarBody, RF_SUS_UPPER_location, 10000.0, 40);

		//Left-Rear suspension
		// (name, mass, diag_inertia, sym_inertia, position)
		rigidBody* LR_wheel = createRigidBody(
			"LR_wheel", 28.05721,
			VEC3D(0.51826350, 0.79352363, 0.51826387),
			VEC3D(0.0, 0.0, 0.0),
			LR_wheel_position);

		rigidBody* LR_SDW = createRigidBody(
			"LR_Suspension_DownWishbon", 5.74921731,
			VEC3D(0.07542368, 0.06583622, 0.13970432),
			VEC3D(0.0, 0.0, 0.0),
			LR_SDW_position);

		rigidBody* LR_SUW = createRigidBody(
			"LR_Suspension_Upwishbon", 5.75592916,
			VEC3D(0.03814835, 0.03106567, 0.06790764),
			VEC3D(0.0, 0.0, 0.0),
			LR_SUW_position);

		rigidBody* LR_SWN = createRigidBody(
		...
		springDamperModel* LR_SUS = createSpringDamperElement(
			...
			CarBody, LR_SUS_UPPER_location, 10000.0, 40);

		// Right-Rear suspension
		// (name, mass, diag_inertia, sym_inertia, position)
		rigidBody* RR_wheel = createRigidBody(
			"RR_wheel", 28.05721,
			VEC3D(0.51826350, 0.79352363, 0.51826387
			),
			VEC3D(0.0, 0.0, 0.0),
		...
		rigidBody* RR_SDW = createRigidBody(
		...
		rigidBody* RR_SUW = createRigidBody(
		...
		rigidBody* RR_SWN = createRigidBody(
		...
		spi = CarBody->toLocal(RR_CarBody_SUW_REV_location - CarBody->getPosition());
		...
		springDamperModel* RR_SUS = createSpringDamperElement(
			...
			CarBody, RR_SUS_UPPER_location, 10000.0, 40);
		...
		spi = CarBody->toLocal(G_CarBody_Fix_location - CarBody_position);
